        ...
        for line in block.splitlines():
            ...
            if line.startswith("|-"):
                ...
                while i < len(cell_data):
                    char = cell_data[i]
                    if char == '|' and i + 1 < len(cell_data) and cell_data[i+1] == '|' and link_cnt == 0 and tpl_cnt == 0:
                        subcells.append("".join(curr).strip())
                        curr = []
                        i += 2
                        continue
                    curr.append(char)
                    if char == '[' and i + 1 < len(cell_data) and cell_data[i+1] == '[':
                        link_cnt += 1
                    elif char == ']' and i + 1 < len(cell_data) and cell_data[i+1] == ']':
                        link_cnt -= 1
                    elif char == '{' and i + 1 < len(cell_data) and cell_data[i+1] == '{':
                        tpl_cnt += 1
                    elif char == '}' and i + 1 < len(cell_data) and cell_data[i+1] == '}':
...
async def run_sets_sync() -> dict:
    ...
    try:
        ...
        logger.info(f"Successfully scraped and synced {len(filtered_sets)} sets to sets.json")
        return {
            "success": True,
            "sets_synced": len(filtered_sets)
        }
