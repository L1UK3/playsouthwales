...
const SuspenseLoader: React.FC<SuspenseLoaderProps> = ({
    ...
    return (
        <div
            className={`flex flex-col items-center justify-center min-h-62.5 w-full animate-[fadeIn_0.4s_ease-out] ${fullPage ? 'fixed inset-0 min-h-screen bg-[#121214] z-9999' : ''}`}
        >
            ...
            {message ? (
                <div className="mt-5 font-sans text-[0.95rem] font-medium text-text-muted tracking-wider animate-pulse">
                    {message}
                </div>
            ) : null}
        </div>
